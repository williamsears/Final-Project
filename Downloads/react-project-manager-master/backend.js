...
app.get('/usernameget', function(req, res) {
  connection.query('SELECT name, email, location FROM Accounts WHERE id="' + req.accountId + '"', function(err, result) {
    if (err) {
      console.log(err);
    } else if (!result || result.length === 0) {
      res.status(404).send();
...
app.put('/Accounts', function(req, res) {
  ...
  Object.keys(req.body).forEach(function(item) {
    if (item === 'name' || item === 'email' || item === 'password' || item === 'location') {
